 ...
 */
if (!function_exists('editech_is_frontpage')) {

    function editech_is_frontpage() {
        return (is_front_page() || is_home());
    }
}

if (!function_exists('editech_is_elementor_activated')) {
    function editech_is_elementor_activated() {
        return defined('ELEMENTOR_VERSION') ? true : false;
    }
}


if (!function_exists('editech_page_enable_breadcrumb')) {
    /**
     * @return bool
     */
    function editech_page_enable_breadcrumb() {

        if (!is_page()) {
            return true;
        }

        $check = editech_get_metabox(get_the_ID(), 'osf_enable_breadcrumb', true);
 ...
 */
function editech_can_show_post_thumbnail() {
    return apply_filters('editech_can_show_post_thumbnail', !post_password_required() && !is_attachment() && has_post_thumbnail());
}

if (!function_exists('editech_get_query')) {
     ...
     * @return WP_Query
     */
    function editech_get_query($args) {
        global $wp_query;
        $default  = array(
            'post_type' => 'post',
        );
...
if (!function_exists('editech_get_placeholder_image')) {

    /**
     * @return string
     */
    function editech_get_placeholder_image() {
        return get_parent_theme_file_uri('/assets/images/placeholder.png');
    }

}

if (!function_exists('editech_is_osf_framework_activated')) {
    /**
     * Query WooCommerce activation
     */
    function editech_is_osf_framework_activated() {
        return class_exists('EditechCore') ? true : false;
    }
}


if (!function_exists('editech_get_metabox')) {
     ...
     * @return bool|mixed
     */
    function editech_get_metabox($id, $key, $default = false) {
...
if (!function_exists('editech_is_blog_archive')) {
    function editech_is_blog_archive() {
        return (is_home() && is_front_page()) || is_category() || is_tag() || is_post_type_archive('post');
    }
}

if (!function_exists('editech_get_excerpt')) {
    function editech_get_excerpt($excerpt_length = 55) {
        global $post;
...
if (!function_exists('editech_do_shortcode')) {
     ...
     *
     */
    function editech_do_shortcode($tag, array $atts = array(), $content = null) {
...
if (!function_exists('editech_nav_menu_social_icons')) {
     ...
     */
    function editech_nav_menu_social_icons($item_output, $item, $depth, $args) {
        // Get supported social icons.
        $social_icons = editech_social_links_icons();
...
add_filter('walker_nav_menu_start_el', 'editech_nav_menu_social_icons', 10, 4);

if (!function_exists('editech_social_links_icons')) {
     ...
     */
    function editech_social_links_icons() {
        ...
        return apply_filters('editech_social_links_icons', $social_links_icons);
    }
}

if (!function_exists('editech_dropdown_icon_to_menu_link')) {
    /**
     ...
     */
    function editech_dropdown_icon_to_menu_link($title, $item, $args, $depth) {
...
add_filter('nav_menu_item_title', 'editech_dropdown_icon_to_menu_link', 10, 4);

if (!function_exists('editech_is_header_builder')) {
    /**
     * @return bool
     */
    function editech_is_header_builder() {
        global $osf_header;
...
if (!function_exists('editech_the_header_builder')) {
    /**
     * @return void
     */
    function editech_the_header_builder() {
        echo editech_get_header_builder_html();
    }
}

if (!function_exists('editech_get_header_builder_html')) {

    /**
     * @return string
     */
    function editech_get_header_builder_html() {
...
if (!function_exists('editech_is_footer_builder')) {
    /**
     * @return bool
     */
    function editech_is_footer_builder() {
        global $osf_footer;
...
if (!function_exists('editech_the_footer_builder')) {
    /**
     * @return void
     */
    function editech_the_footer_builder() {
        echo editech_get_footer_builder_html();
    }
}


if (!function_exists('editech_get_footer_builder_html')) {
    function editech_get_footer_builder_html() {
...
if (!function_exists('editech_license_get_option')) {
    function editech_license_get_option($key = '', $default = false) {
...
function editech_social_share() {
    if (editech_is_osf_framework_activated() && get_theme_mod('osf_socials', true)) {
        $template = WP_PLUGIN_DIR . '/editech-core/templates/socials.php';
...
function editech_social_heading() {
    return esc_html__('Share:', 'editech');
}

add_filter('osf_social_heading', 'editech_social_heading');

function editech_get_post_link($taxonomy = 'category', $post_type = ['any']) {
...
if (!function_exists('editech_fnc_related_post')) {
    function editech_fnc_related_post($relate_count = 4, $posttype = 'post', $taxonomy = 'category') {
        ...
        if ($related->have_posts()) {
            ...
            echo '<h3 class="related-heading">' . esc_html__('You might also like', 'editech') . '</h3>';
...
if (!function_exists('editech_get_review_counting')) {
    function editech_get_review_counting() {
...
add_action('editech_single_after_content', 'editech_fnc_nav_post', 10);
if (!function_exists('editech_fnc_nav_post')) {
    function editech_fnc_nav_post() {
        $obj       = editech_get_post_link('category', 'post');
                ...
                <?php if (!empty($prev_link)): ?>
                    <div class="previous-nav">
                        <div class="nav-content">
                            ...
                            <div class="nav-link">
                                <div class="nav-title"><?php esc_html_e('Prev', 'editech'); ?></div>
                ...
                <?php if (!empty($next_link)): ?>
                    <div class="next-nav">
                        <div class="nav-content">
                            ...
                            <div class="nav-link">
                                <div class="nav-title"><?php esc_html_e('Next', 'editech'); ?></div>
...
function editech_get_the_category_list($separator = '', $parents = '', $post_id = false) {
    global $wp_rewrite;
    ...
    if (empty($categories)) {
        /** This filter is documented in wp-includes/category-template.php */
        return apply_filters('the_category', __('Uncategorized', 'editech'), $separator, $parents);
    }
...
add_filter('wp_list_categories', 'editech_add_span_cat_count');
function editech_add_span_cat_count($links) {
