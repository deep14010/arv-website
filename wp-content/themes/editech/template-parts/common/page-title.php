...
if (!editech_page_enable_breadcrumb()) {
    return;
}
...
$otf_home  = esc_html__('Home', 'editech');
$otf_blog  = esc_html__('Blog', 'editech');
$otf_shop  = esc_html__('Shop', 'editech');
$otf_title = '';
...
if (!function_exists('bcn_display')) {
    ...
    if (!is_front_page()) {
        ...
        if (is_home()) {
            ...
            $otf_title = esc_html__('Day', 'editech');
            // Day archive
            ...
            if (!empty($author->ID)) {
                ...
                $otf_title    = esc_html__('Author: ', 'editech') . esc_html($otf_userdata->display_name);
                // Get the author information
                $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Author: ', 'editech') . '<a href="' . get_author_posts_url($otf_userdata->ID, $otf_userdata->nice_name) . '">' . esc_html($otf_userdata->display_name) . '</a></li>';
            }

        } elseif (get_query_var('paged')) {
            // Paginated archives
            $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Page', 'editech') . ' ' . get_query_var('paged', '1') . '</li>';

        } elseif (is_search()) {
            $otf_title  = esc_html__('Search', 'editech');
            $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Keyword: ', 'editech') . get_search_query() . '</li>';

        } elseif (is_404()) {
            $otf_title  = esc_html__('Error 404', 'editech');
            $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Error 404', 'editech') . '</li>';
        }
    }
    $otf_output .= '</ul>';
} elseif
(!is_front_page()) {
    if (is_home()) {
        ...
        global $author;
        if (!empty($author->ID)) {
            $otf_title = esc_html__('Author', 'editech');
        }
